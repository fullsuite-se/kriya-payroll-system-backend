...
export const getAttendances = async (req: Request, res: Response) => {
    ...
    if (!company_id) return res.status(400).json({ message: "Failed to fetch attendance", error: "Missing company_id" });

    try {
        const attendances = await findAllAttendances(company_id, employee_id, from, to);
        return res.status(200).json({ message: "Attendance successfully fetched", attendances });
    } catch (error) {
        return res.status(400).json({ message: "Failed to fetch attendance", error: getErrorMessage(error) });
    }
};
...
export const addAttendance = async (req: Request, res: Response) => {
    ...
    if (!parsedAttendance.success) return res.status(400).json({ message: "Failed to add attendance. cat", error: parsedAttendance.error?.issues });

    if (!company_id) return res.status(400).json({ message: "Failed to fetch attendance", error: "Missing company_id" });

    try {
        const attendance = await addOneAttendance(company_id, parsedAttendance.data);
        return res.status(201).json({ message: "Attendance added successfully", attendance });
    } catch (error) {
        return res.status(500).json({ message: "Failed to add attendance", error: getErrorMessage(error) });
    }
};
...
export const updateAttendance = async (req: Request, res: Response) => {
    ...
    try {
        const attendance = await updateOneAttendance(employee_attendance_id, parsedAttendance.data);
